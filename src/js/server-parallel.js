...
if (cluster.isMaster) {
  ...
  if (
    ...
    console.log(
      wrapText(
        "Starts fairyground HTTP and WebSocket server.",
        consolewidth,
        0,
      ),
    );
    ...
    console.log(
      wrapText(
        "node server-parallel.js [--number-workers <int>]",
        consolewidth,
        0,
      ),
    );
    ...
    console.log(
      wrapText("--number-workers <numWorkers: int>", consolewidth, 0),
    );
    console.log(
      wrapText(
        "Sets the count of workers for HTTP server. (Default: CPU thread count)",
        consolewidth,
        10,
      ),
    );
    ...
    console.log(wrapText("node server-parallel.js", consolewidth, 0));
    console.log(
      wrapText("node server-parallel.js --number-workers 1", consolewidth, 0),
    );
    console.log(
      wrapText("node server-parallel.js --number-workers 8", consolewidth, 0),
    );
    console.log("");
    process.exit(0);
  }
  ...
  if (args.includes("--number-workers")) {
    let num = Number.parseInt(args[args.indexOf("--number-workers") + 1], 10);
    if (num > 0) {
      numworkers = num;
    } else {
      console.error(
        "The count of HTTP server workers must be a positive integer.",
      );
      process.exit(1);
    }
  }

  const NumberParallel = numworkers || os.availableParallelism();
  let i = 0;
  console.log("============================================");
  ...
  console.log(`[HTTP Server] Master process (${process.pid}) is running.`);
  console.log(
    "[HTTP Server] Using cluster mode. Parallelism count:",
    NumberParallel,
  );
  ...
  cluster.on("exit", (worker, code, signal) => {
    console.log(
      `[HTTP Server] A worker process (${worker.process.pid}) exited unexpectedly (Code ${code}, Signal ${signal}). Restarting...`,
    );
    cluster.fork();
  });
} else {
  ...
  app.all("/", function (req, res) {
    console.log("[HTTP Server Worker %d] Redirect to index page.", process.pid);
  ...
  const httpserver = app.listen(port, function () {
    ...
    console.log(
      "[HTTP Server Worker %d] Server is up at http://localhost:%s",
      process.pid,
      port,
    );
  });
...
if (cluster.isMaster) {
  ...
  const wss = new WebSocketServer({ port: port + 1 }, () => {
    console.log(
      "[WebSocket Server] Server is up at ws://localhost:%s",
      port + 1,
    );
  });
  ...
  class Engine {
    constructor(
      ...
      if (
        ...
        !(WebSocketConnection instanceof WebSocket)
      ) {
        throw TypeError();
      }
    ...
    Load(LoadFinishCallBack, LoadFailureCallBack) {
      ...
      console.log(
        ...
        nodePath.basename(this.CommandParts[0]).trim(),
        " loading for ",
        this.Color,
        `(ID:${this.ID}).`,
      );
